...
const BISTRO_CLOSING_PARTIAL_KEY = 'bistro_closing_partial_settlement';

const orderScreenManagerButtonClass =
  'w-full px-4 py-2 rounded-lg border font-semibold transition-colors border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 disabled:pointer-events-none disabled:cursor-not-allowed disabled:border-gray-200 disabled:bg-gray-100 disabled:text-gray-400 disabled:hover:bg-gray-100';
...
const OrderSetupPage = () => {
  ...
  const [bistroPartialClosingSettlement, setBistroPartialClosingSettlement] = useState(false);
  const [bistroClosingSaving, setBistroClosingSaving] = useState(false);
  const [bistroClosingMsg, setBistroClosingMsg] = useState('');

  const loadClosingSettings = useCallback(async () => {
    try {
      const [layoutRes, appRes] = await Promise.all([
        fetch(`${API_URL}/layout-settings`),
        fetch(`${API_URL}/app-settings/${encodeURIComponent(BISTRO_CLOSING_PARTIAL_KEY)}`),
      ]);
      if (layoutRes.ok) {
        const result = await layoutRes.json();
        if (result?.success && result?.data) {
          setSelectServerOnEntry(result.data.selectServerOnEntry ?? false);
        }
      }
      if (appRes.ok) {
        const j = (await appRes.json()) as { value?: string };
        const v = String(j?.value || '').trim().toLowerCase();
        setBistroPartialClosingSettlement(v === '1' || v === 'true' || v === 'yes' || v === 'on');
      } else {
        setBistroPartialClosingSettlement(false);
      }
    } catch {
      setBistroPartialClosingSettlement(false);
    }
  ...
  const saveBistroPartialClosingSettlement = useCallback(async (enabled: boolean) => {
    setBistroClosingSaving(true);
    setBistroClosingMsg('');
    try {
      const res = await fetch(`${API_URL}/app-settings/${encodeURIComponent(BISTRO_CLOSING_PARTIAL_KEY)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          value: enabled ? '1' : '0',
          description:
            'Bistro only: Z-report / shift / day close count approved payments even if order is not fully paid (unpaid balance excluded).',
        }),
      });
      if (!res.ok) throw new Error('save failed');
      setBistroPartialClosingSettlement(enabled);
      setBistroClosingMsg('Saved.');
      window.setTimeout(() => setBistroClosingMsg(''), 2500);
    } catch {
      setBistroClosingMsg('Could not save. Check network or try again.');
    } finally {
      setBistroClosingSaving(false);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-5">
      <div className="max-w-4xl mx-auto px-4 leading-snug">
        <div className="mb-4 text-center">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">Order Screen Setup</h1>
          <p className="mt-1 text-sm text-gray-600">Select order channel and menu to configure the order screen</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div className="bg-white rounded-lg shadow-md p-4">
            <h2 className="text-xl font-semibold text-gray-900 mb-2">Order Channel Selection</h2>
              ...
              {orderTypes.map((type) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-2.5 text-left rounded-lg border-2 transition-all duration-200 ${
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <div className="flex-1">
                        ...
                        {/* 연결된 메뉴 표시 */}
                        {connectedMenu && (
                          <p className="text-sm font-medium text-green-600 mt-0.5 flex flex-wrap items-center gap-y-0.5">
          ...
          <div className="bg-white rounded-lg shadow-md p-4">
            {selectedOrderType === 'online' ? (
              <div className="space-y-2">
                <h2 className="text-xl font-semibold text-gray-900 mb-0.5">Bistro Mode</h2>
              ...
              <>
                <h2 className="text-xl font-semibold text-gray-900 mb-2">Menu Selection</h2>
              ...
              <div className="space-y-2.5">
                ...
                <div className="relative">
                  <label htmlFor="menu-select" className="block text-sm font-medium text-gray-700 mb-1">
                    Select a menu
                  </label>
                  <select
                    id="menu-select"
                ...
                {selectedOrderType === 'togo' && (
                  <div className="space-y-2.5">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Customer Info Timing</label>
                      <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">
                      <div className="p-2.5">
                        <label className="block text-sm font-medium text-gray-700 mb-1">TOGO Panel (Right Side)</label>
                        <div className="grid grid-cols-2 gap-2">
                      ...
                      >
                        <div className="min-h-0 overflow-hidden">
                          <div className="border-t border-gray-100 bg-gray-50/60 px-3 pb-2 pt-2">
                            <label className="block text-sm font-medium text-gray-700 mb-0.5">
                              Table map / TOGO panel width
                            </label>
                            <p className="text-xs text-gray-500 mb-1.5">Table map (left) : TOGO panel (right).</p>
                    ...
                    {/* TOGO Button */}
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Pickup Order Buttons (Header)</label>
                      <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="pt-1">
                      <button
                        ...
                        disabled={isSaving}
                        className={`w-full py-2.5 rounded-lg font-semibold text-sm transition-all ${isSaving ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700 shadow-lg'}`}
                      >
                ...
                <div className="pt-2">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      ...
                      className={`mt-2 ${orderScreenManagerButtonClass}`}
                    >
                      Open Order Screen Manager (Reorder via Drag & Drop)
                    </button>
                  </div>
        ...
        {canContinue && (
          <div className="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-3">
            <div className="flex items-center justify-center space-x-3 text-blue-800 flex-wrap gap-x-3 gap-y-1 text-sm">
        ...
        {activeSection === 'closing' && (
          <div className="bg-white rounded-lg shadow-md p-4 max-w-2xl">
            <h2 className="text-xl font-semibold text-gray-900 mb-1">Closing Settings</h2>
            <p className="text-sm text-gray-500 mb-3 leading-snug">Configure settings for Shift Close and Day Close</p>

            {/* Bistro: Z / shift / day — partial payments vs legacy (Business Profile service type must be BISTRO) */}
            <div className="mb-4 rounded-xl border border-violet-300/70 bg-violet-100/90 p-3 space-y-2 shadow-sm">
              <h3 className="rounded-lg border border-violet-400/35 bg-violet-300/70 px-3 py-2 text-sm font-bold text-violet-950 shadow-sm">
                Bistro — Z / shift / day close
              </h3>
              <div className="space-y-2 rounded-lg border border-violet-200/80 bg-white/85 p-3">
                <p className="text-xs text-gray-600 leading-snug">
                  <strong>Legacy</strong> — closing uses only checks that are <strong>fully paid</strong>.
                </p>
                <p className="text-xs text-gray-600 leading-snug">
                  <strong>Partial</strong> — closing uses <strong>money already collected</strong>; what is still owed waits.
                  After Day Close, <strong>unpaid tabs stay on the table map</strong>.
                </p>
                <p className="text-xs text-amber-800 font-medium leading-snug">
                  Bistro only — Business Profile must be set to <strong>BISTRO</strong>.
                </p>
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    type="button"
                    disabled={bistroClosingSaving}
                    onClick={() => void saveBistroPartialClosingSettlement(false)}
                    className={`px-4 py-2 rounded-lg border-2 text-sm font-semibold transition-all ${
                      !bistroPartialClosingSettlement
                  ...
                  <button
                    type="button"
                    disabled={bistroClosingSaving}
                    onClick={() => void saveBistroPartialClosingSettlement(true)}
                    className={`px-4 py-2 rounded-lg border-2 text-sm font-semibold transition-all ${
                      bistroPartialClosingSettlement
                  ...
                  >
                    Partial payments in close
                  </button>
                </div>
                {bistroClosingMsg ? (
                  <p
                    className={`text-xs font-medium ${
                      bistroClosingMsg.includes('Could not') ? 'text-red-600' : 'text-emerald-700'
                    }`}
                  >
                    {bistroClosingMsg}
                  </p>
            ...
            <div className="mb-4 rounded-xl border border-sky-300/70 bg-sky-100/90 p-3 space-y-3 shadow-sm">
              <h3 className="rounded-lg border border-sky-400/35 bg-sky-300/70 px-3 py-2 text-sm font-bold text-sky-950 shadow-sm">
                Order Entry
              </h3>

              {/* Select Server on Entry Toggle */}
              <div className="flex items-center justify-between rounded-lg border border-sky-200/80 bg-white/85 p-3">
                <div className="flex-1 mr-4">
                  <p className="font-medium text-gray-900 text-sm leading-snug">Select Server</p>
            ...
            <div className="rounded-xl border border-teal-300/70 bg-teal-100/90 p-3 space-y-3 shadow-sm">
              <h3 className="rounded-lg border border-teal-400/35 bg-teal-300/70 px-3 py-2 text-sm font-bold text-teal-950 shadow-sm">
                Shift Close
              </h3>

              {/* Require server before Shift Close */}
              <div className="flex items-center justify-between rounded-lg border border-teal-200/80 bg-white/85 p-3">
                <div className="flex-1 mr-4">
                  <p className="font-medium text-gray-900 text-sm leading-snug">Require server before Shift Close</p>
                  <p className="text-xs text-gray-500 mt-0.5 leading-snug">
              ...
              {shiftServerSelection && (
                <div className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg space-y-1.5">
                  <p className="text-xs font-bold text-slate-700 uppercase tracking-wide leading-snug">Unpaid order transfer</p>
                  <ul className="text-xs text-slate-600 space-y-1 list-disc list-inside pl-0.5 leading-snug">
