...
import { getContrastingTextColor, isHexColor, getSelectedButtonColor, getDarkerHexColor } from '../../utils/colorUtils';
...
= ({ item, layoutSettings, itemColors, isSelected, multiSelectMode, onClick, onContext, buttonWidth, emphasize, isSoldOut = false, soldOutInfo, lockLayout }) => {
  const { attributes, listeners, setNodeRef, transform, isDragging } = useSortable({ id: item.id, disabled: lockLayout });
  ...
  return (
    ...
    >
      ...
      <div className={`font-medium text_center break_words leading-tight ${layoutSettings.menuFontBold ? 'font-bold' : 'font-normal'} ${isSoldOut ? 'line-through decoration-2' : ''}`} style={{ fontSize: `${layoutSettings.menuFontSize}px`, letterSpacing: '0.1px' }}>
...
const SortableEmptyCell: React.FC<{ id: string; height: number }> = ({ id, height }) => {
  const { setNodeRef } = useSortable({ id, disabled: true });
  return (
    <div
      ref={setNodeRef}
      className="w-full h-full rounded-xl border border-dashed border-gray-300"
...
const MenuItemGrid: React.FC<MenuItemGridProps> = ({
  ...
  const shouldShowEmptySlots = showEmptySlots !== false;
  ...
  // Build capacity-limited layout ids including EMPTY slots - limit to actual items only (no extra empty rows)
  const availableIds = filteredMenuItems.map(it => String(it.id));
  const capacity = Math.max(1, availableIds.length);
  ...
  const layoutIds = shouldShowEmptySlots ? kept.slice(0, capacity) : kept.slice();
    ...
    <DndContext 
      ...
      onDragStart={({active}) => {
        ...
        if (mergyActive && isMergedSelected && currentMergyGroupId) {
          ...
          if (it && it.category) {
            const ids = buildIdsForCategory(it.category);
            mergedDragRef.current = { ids, category: it.category };
          } else {
      ...
      onDragEnd={(e) => {
        ...
        // If dragging extra buttons, defer to parent handler
        ...
        if (typeof onMenuGridReorder === 'function') {
          onMenuGridReorder({ oldIndex, newIndex, ids, category: categoryForReorder });
        }
        setActiveMenuId(null);
      }}
    >
      ...
      {mergyActive && isMergedSelected && currentMergyGroupId ? (
        <div className="space-y-2">
          ...
          {(() => {
            ...
            return g.categoryNames.map((catName, catIdx) => {
              const catItems = menuItems.filter(mi => mi.category === catName);
              if (catItems.length === 0) return null;
              const headerOffset = 0;
              const standardGapPx = 2;
              const extraSeparationPx = 4; // reduced separation between merged category blocks
              const catIds = buildIdsForCategory(catName);
        ...
        <div className="space-y-3 overflow-auto pb-16">
          ...
          <SortableContext items={layoutIds} strategy={rectSortingStrategy}>
            {allItems.map((item, itemIndex) => {
              ...
              return String(item.id).startsWith('EMPTY:') ? (
                <SortableEmptyCell key={item.id} id={String(item.id)} height={layoutSettings.menuItemHeight} />
              ) : (
      ...
      {/* Extra Buttons - Fixed at bottom of menu area */}
      {Array.isArray(extraItems) && extraItems.length > 0 && (() => {
        const extraCount = Math.min(extraItems.length, 3);
        const emptyCount = Math.max(0, maxCols - extraCount);
        return (
          <div className="flex-shrink-0 px-1 pt-1" style={{ display: 'grid', gridTemplateColumns: `repeat(${maxCols}, minmax(0, 1fr))`, gap: '0.25rem' }}>
            {/* Empty cells to push Extra Buttons to the right */}
            {Array.from({ length: emptyCount }).map((_, i) => (
              <div key={`empty-extra-${i}`} />
            ))}
      ...
      <DragOverlay dropAnimation={{ duration: 220, easing: 'cubic-bezier(0.22, 1, 0.36, 1)', sideEffects: defaultDropAnimationSideEffects({ styles: { active: { opacity: '0.5' } } }) }}>
        {activeMenuId ? (() => {
          const it = filteredMenuItems.find(i => String(i.id) === activeMenuId);
          if (!it) return null;
          const isSel = selectedMenuItemId === it.id;
          const bgClass = itemColors[it.id] || (it as any).color || layoutSettings.menuDefaultColor;
          const isHex = isHexColor(bgClass);
          const selectedBg = '#1E3A8A';
          const selectedIsHex = isHexColor(selectedBg);
          return (
            <div className={`rounded-xl border flex items-center justify_center ${getContrastingTextColor(isSel ? selectedBg : bgClass)} ${isSel && !selectedIsHex ? selectedBg : ''}`}
                 style={{ height: `${layoutSettings.menuItemHeight}px`, width: `${buttonWidth}px`, backgroundColor: isSel ? (selectedIsHex ? selectedBg : undefined) : (isHex ? bgClass : undefined), backgroundImage: 'linear-gradient(180deg, rgba(255,255,255,0.28), rgba(255,255,255,0.06)), linear-gradient(180deg, rgba(255,255,255,0.08), rgba(0,0,0,0.18))', border: '1px solid rgba(255,255,255,0.18)', boxShadow: (isSel ? 'inset 0 3px 10px rgba(0,0,0,0.55), 0 1px 0 rgba(255,255,255,0.15) inset' : '0 4px 10px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.3)'), borderBottom: (isSel ? '2px solid rgba(0,0,0,0.25)' : '2px solid rgba(0,0,0,0.2)') }}>
              <div className={`${layoutSettings.menuFontBold ? 'font-bold' : 'font-normal'} text_center break_words`}>{(layoutSettings.useShortName && (it as any).short_name) ? (it as any).short_name : it.name}</div>
            </div>
          );
        })() : null}
      </DragOverlay>
