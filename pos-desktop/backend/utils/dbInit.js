...
async function initDatabase(db) {
  ...
  try {
    ...
    // Migration: per-device graphic scale (bitmap/graphic mode)
    if (!pColNames.includes('graphic_scale')) {
      try {
        await dbRun("ALTER TABLE printers ADD COLUMN graphic_scale REAL DEFAULT 1.0");
        console.log('[dbInit] Added graphic_scale column to printers');
      } catch (e) { console.error('[dbInit] printers graphic_scale:', e.message); }
    }
    ...
    const oiMigrations = [
      ...
      { col: 'split_numerator', sql: "ALTER TABLE order_items ADD COLUMN split_numerator INTEGER" },
    ...
    // 10. DAILY CLOSINGS & SHIFT CLOSE (중간정산)
    // NOTE: daily-closings route expects a session-based schema.
    // Older builds created legacy tables (closing_date/shift_date) that break current inserts due to NOT NULL constraints.
    const tableExists = async (name) => {
      const row = await dbGet("SELECT name FROM sqlite_master WHERE type='table' AND name = ?", [name]);
      return !!row;
    };

    const migrateDailyClosingsIfLegacy = async () => {
      const info = await dbAll("PRAGMA table_info(daily_closings)");
      if (!info || info.length === 0) return null;
      const cols = info.map(c => String(c.name));
      const isLegacy = cols.includes('closing_date') || cols.includes('closing_type') || cols.includes('data');
      if (!isLegacy) return null;

      const baseName = 'daily_closings_legacy';
      const legacyName = (await tableExists(baseName)) ? `${baseName}_${Date.now()}` : baseName;
      await dbRun(`ALTER TABLE daily_closings RENAME TO ${legacyName}`);
      console.log(`[dbInit] Renamed legacy daily_closings -> ${legacyName}`);
      return legacyName;
    };

    const ensureDailyClosings = async () => {
      const legacyName = await migrateDailyClosingsIfLegacy();
      ...
      // Best-effort backfill from legacy schema (keep history without blocking new sessions).
      if (legacyName && await tableExists(legacyName)) {
        try {
          const legacyCols = (await dbAll(`PRAGMA table_info(${legacyName})`)).map(c => String(c.name));
          const hasData = legacyCols.includes('data');
          await dbRun(
            `INSERT INTO daily_closings (session_id, date, closed_at, status, notes, created_at, updated_at)
             SELECT
               'LEGACY-' || REPLACE(closing_date, '-', '') || '-' || CAST(id AS TEXT),
               closing_date,
               COALESCE(created_at, CURRENT_TIMESTAMP),
               'closed',
               ${hasData ? 'COALESCE(data, NULL)' : 'NULL'},
               COALESCE(created_at, CURRENT_TIMESTAMP),
               COALESCE(created_at, CURRENT_TIMESTAMP)
             FROM ${legacyName}
             WHERE closing_date IS NOT NULL`
          );
          console.log('[dbInit] Backfilled legacy daily_closings into session-based schema');
        } catch (e) {
          console.error('[dbInit] daily_closings legacy backfill failed:', e.message);
        }
      }
    };

    const migrateShiftClosingsIfLegacy = async () => {
      const info = await dbAll("PRAGMA table_info(shift_closings)");
      if (!info || info.length === 0) return null;
      const cols = info.map(c => String(c.name));
      const isLegacy = cols.includes('shift_date') || cols.includes('shift_label') || cols.includes('data');
      if (!isLegacy) return null;

      const baseName = 'shift_closings_legacy';
      const legacyName = (await tableExists(baseName)) ? `${baseName}_${Date.now()}` : baseName;
      await dbRun(`ALTER TABLE shift_closings RENAME TO ${legacyName}`);
      console.log(`[dbInit] Renamed legacy shift_closings -> ${legacyName}`);
      return legacyName;
    };

    const ensureShiftClosings = async () => {
      const legacyName = await migrateShiftClosingsIfLegacy();
      ...
      await dbRun(`CREATE INDEX IF NOT EXISTS idx_shift_closings_session_shift
        ON shift_closings(session_id, shift_number)`);

      if (legacyName && await tableExists(legacyName)) {
        try {
          const legacyCols = (await dbAll(`PRAGMA table_info(${legacyName})`)).map(c => String(c.name));
          const hasOpenedAt = legacyCols.includes('opened_at');
          const hasClosedAt = legacyCols.includes('closed_at');
          const hasShiftLabel = legacyCols.includes('shift_label');
          await dbRun(
            `INSERT INTO shift_closings (session_id, shift_number, shift_start, shift_end, closed_by, created_at)
             SELECT
               'LEGACY-' || REPLACE(shift_date, '-', '') || '-' || CAST(id AS TEXT),
               1,
               ${hasOpenedAt ? 'opened_at' : 'NULL'},
               ${hasClosedAt ? 'closed_at' : 'NULL'},
               ${hasShiftLabel ? 'shift_label' : 'NULL'},
               COALESCE(created_at, CURRENT_TIMESTAMP)
             FROM ${legacyName}
             WHERE shift_date IS NOT NULL`
          );
          console.log('[dbInit] Backfilled legacy shift_closings into new schema');
        } catch (e) {
          console.error('[dbInit] shift_closings legacy backfill failed:', e.message);
        }
      }
    };
    ...
    // ============================================================
    // SERVER MID-SHIFT SETTLEMENT (Server-level accounting sessions)
    // ============================================================
    await dbRun(`CREATE TABLE IF NOT EXISTS server_shifts (
      shift_id INTEGER PRIMARY KEY AUTOINCREMENT,
      server_id TEXT NOT NULL,
      clock_record_id INTEGER,
    ...
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_server_shifts_server_date
      ON server_shifts(server_id, business_date)`);
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_server_shifts_status
      ON server_shifts(status)`);

    await dbRun(`CREATE TABLE IF NOT EXISTS server_settlements (
      ...
      initiated_by TEXT, -- server | manager | system
      ...
      card_tips REAL DEFAULT 0,
      total_tip REAL DEFAULT 0,

      safe_drop_total REAL DEFAULT 0,
      cash_refund_total REAL DEFAULT 0,
      ...
      approval_status TEXT DEFAULT 'auto', -- auto | manager_required | approved | rejected
      approved_by TEXT,
      approved_at TEXT,

      created_at TEXT DEFAULT (datetime('now')),
      updated_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY (shift_id) REFERENCES server_shifts(shift_id)
    )`);
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_server_settlements_server_date
      ON server_settlements(server_id, business_date)`);
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_server_settlements_shift
      ON server_settlements(shift_id)`);
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_server_settlements_status
      ON server_settlements(status)`);

    await dbRun(`CREATE TABLE IF NOT EXISTS settlement_payments (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      ...
      created_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY (settlement_id) REFERENCES server_settlements(settlement_id) ON DELETE CASCADE
    )`);
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_settlement_payments_settlement
      ON settlement_payments(settlement_id)`);

    await dbRun(`CREATE TABLE IF NOT EXISTS server_cash_drops (
    ...
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_server_cash_drops_shift
      ON server_cash_drops(shift_id)`);
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_server_cash_drops_server_date
      ON server_cash_drops(server_id, timestamp)`);

    await dbRun(`CREATE TABLE IF NOT EXISTS delivery_online_tip_pool (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      business_date TEXT NOT NULL,
      tip_amount REAL NOT NULL DEFAULT 0,
      source_type TEXT, -- online | delivery | mixed | unknown
      created_at TEXT DEFAULT (datetime('now'))
    )`);
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_delivery_online_tip_pool_date
      ON delivery_online_tip_pool(business_date)`);

    // Dedicated audit trail for settlement & approvals (do NOT delete records)
    ...
    await dbRun(`CREATE INDEX IF NOT EXISTS idx_audit_log_ref
