/**
 * 1) order_type = ONLINE 인 주문은 모두 삭제 (자식 행 포함).
 * 2) TOGO: DB에 개발/매장 구분 필드가 없어, 기본은 TOGO 전부 유지.
 *    TOGO 일부 삭제 시:
 *    --togo-delete-before YYYY-MM-DD   해당 로컬 날짜 "이전" 생성분만 삭제
 *    --togo-delete-from A --togo-delete-to B   로컬 날짜 닫힌 구간 [A,B] 삭제
 *
 * Usage:
 *   node scripts/purge-online-and-togo-selective.js "C:\path\web2pos.db" --dry-run
 *   node scripts/purge-online-and-togo-selective.js "C:\path\web2pos.db" --execute
 *   node scripts/purge-online-and-togo-selective.js "C:\path\web2pos.db" --execute --togo-delete-before 2026-04-01
 */
const fs = require('fs');
const sqlite3 = require('sqlite3').verbose();

function parseArgs(argv) {
  const out = {
    dbPath: null,
    dryRun: false,
    execute: false,
    togoDeleteBefore: null,
    togoDeleteFrom: null,
    togoDeleteTo: null,
  };
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === '--dry-run') out.dryRun = true;
    else if (a === '--execute') out.execute = true;
    else if (a === '--togo-delete-before') out.togoDeleteBefore = argv[++i];
    else if (a === '--togo-delete-from') out.togoDeleteFrom = argv[++i];
    else if (a === '--togo-delete-to') out.togoDeleteTo = argv[++i];
    else if (!a.startsWith('--')) out.dbPath = a;
  }
  return out;
}

const opts = parseArgs(process.argv.slice(2));

if (!opts.dbPath || !fs.existsSync(opts.dbPath)) {
  console.error(
    'Usage: node scripts/purge-online-and-togo-selective.js "<web2pos.db>" --dry-run|--execute [--togo-delete-before YYYY-MM-DD | --togo-delete-from A --togo-delete-to B]'
  );
  process.exit(1);
}

if (opts.dryRun === opts.execute) {
  console.error('정확히 하나만: --dry-run 또는 --execute');
  process.exit(1);
}

if (
  (opts.togoDeleteFrom && !opts.togoDeleteTo) ||
  (!opts.togoDeleteFrom && opts.togoDeleteTo)
) {
  console.error('--togo-delete-from 과 --togo-delete-to 는 쌍으로 지정하세요.');
  process.exit(1);
}

function run(db, sql, params = []) {
  return new Promise((resolve, reject) => {
    db.run(sql, params, function onRun(err) {
      if (err) reject(err);
      else resolve(this.changes);
    });
  });
}

function all(db, sql, params = []) {
  return new Promise((resolve, reject) => {
    db.all(sql, params, (err, rows) => {
      if (err) reject(err);
      else resolve(rows || []);
    });
  });
}

async function tableExists(db, name) {
  const row = await new Promise((resolve, reject) => {
    db.get(
      "SELECT name FROM sqlite_master WHERE type='table' AND name=?",
      [name],
      (err, r) => (err ? reject(err) : resolve(r))
    );
  });
  return !!row;
}

async function main() {
  const db = new sqlite3.Database(opts.dbPath);

  const onlineRows = await all(
    db,
    `SELECT id FROM orders WHERE UPPER(COALESCE(order_type, '')) = 'ONLINE'`,
    []
  );
  const onlineIds = onlineRows.map((r) => r.id);

  let togoRows = [];
  if (opts.togoDeleteBefore) {
    togoRows = await all(
      db,
      `SELECT id, order_number, status, COALESCE(created_at, updated_at) AS ts FROM orders
       WHERE UPPER(COALESCE(order_type, '')) = 'TOGO'
         AND date(COALESCE(created_at, updated_at)) < date(?)`,
      [opts.togoDeleteBefore]
    );
  } else if (opts.togoDeleteFrom && opts.togoDeleteTo) {
    togoRows = await all(
      db,
      `SELECT id, order_number, status, COALESCE(created_at, updated_at) AS ts FROM orders
       WHERE UPPER(COALESCE(order_type, '')) = 'TOGO'
         AND date(COALESCE(created_at, updated_at)) >= date(?)
         AND date(COALESCE(created_at, updated_at)) <= date(?)`,
      [opts.togoDeleteFrom, opts.togoDeleteTo]
    );
  }

  const togoIds = togoRows.map((r) => r.id);
  const idSet = new Set([...onlineIds, ...togoIds]);
  const ids = [...idSet];

  console.log('삭제 예정 ONLINE:', onlineIds.length);
  console.log('삭제 예정 TOGO:', togoIds.length);
  console.log('합계(중복 제거) orders:', ids.length);

  if (opts.dryRun) {
    if (togoRows.length) {
      console.log('\n[삭제 대상 TOGO 샘플 최대 20]');
      togoRows.slice(0, 20).forEach((r) => {
        console.log(`  id=${r.id} #${r.order_number} ${r.status} ${r.ts}`);
      });
    } else if (!opts.togoDeleteBefore && !(opts.togoDeleteFrom && opts.togoDeleteTo)) {
      console.log('\n[TOGO] 삭제 옵션 없음 → TOGO는 유지됩니다.');
    }
    console.log('\n--dry-run: DB 변경 없음');
    db.close();
    return;
  }

  if (ids.length === 0) {
    console.log('삭제할 주문 없음');
    db.close();
    return;
  }

  const backupPath = `${opts.dbPath}.bak-${new Date().toISOString().replace(/[:.]/g, '-')}`;
  fs.copyFileSync(opts.dbPath, backupPath);
  console.log('백업:', backupPath);

  const placeholders = ids.map(() => '?').join(',');

  const deleteFrom = async (table, clause = `order_id IN (${placeholders})`) => {
    if (!(await tableExists(db, table))) return 0;
    try {
      return await run(db, `DELETE FROM ${table} WHERE ${clause}`, ids);
    } catch (e) {
      if (e.message && e.message.includes('no such column')) return 0;
      throw e;
    }
  };

  await run(db, 'BEGIN IMMEDIATE');
  try {
    await deleteFrom('order_items');
    await deleteFrom('payments');
    await deleteFrom('tips');
    await deleteFrom('order_adjustments');
    await deleteFrom('order_guest_status');
    await deleteFrom('delivery_orders');
    if (await tableExists(db, 'refund_items') && (await tableExists(db, 'refunds'))) {
      await run(
        db,
        `DELETE FROM refund_items WHERE refund_id IN (SELECT id FROM refunds WHERE order_id IN (${placeholders}))`,
        ids
      );
    }
    await deleteFrom('refunds');
    if (await tableExists(db, 'voids')) {
      await run(
        db,
        `DELETE FROM void_lines WHERE void_id IN (SELECT id FROM voids WHERE order_id IN (${placeholders}))`,
        ids
      );
      await run(db, `DELETE FROM voids WHERE order_id IN (${placeholders})`, ids);
    }
    if (await tableExists(db, 'table_map_elements')) {
      try {
        await run(
          db,
          `UPDATE table_map_elements SET current_order_id = NULL WHERE current_order_id IN (${placeholders})`,
          ids
        );
      } catch (_) {}
    }
    const n = await run(db, `DELETE FROM orders WHERE id IN (${placeholders})`, ids);
    await run(db, 'COMMIT');
    console.log('완료: orders 삭제', n);
  } catch (err) {
    await new Promise((r) => db.run('ROLLBACK', () => r()));
    console.error('ROLLBACK:', err.message);
    process.exitCode = 1;
  }
  db.close();
}

main().catch((e) => {
  console.error(e);
  process.exit(1);
});
