...
module.exports = (db) => {
  ...
  // 단일 날짜: 달력일 잠정 집계 + 해당일과 겹치는 각 데이세션(오프닝~클로징)별 전체 리포트
  // ============================================================
  const { buildSalesReportPayload, calendarCfg, rangeCfg } = require('../utils/salesReportPayload');
  ...
  router.get('/sales-report', async (req, res) => {
    try {
      ...
      const basePayload = await buildSalesReportPayload(dbGet, dbAll, calendarCfg(startDate, endDate));
      const period = { startDate, endDate };

      const singleCalendarDay = String(startDate).trim() === String(endDate).trim();
      if (!singleCalendarDay) {
        return res.json({ success: true, period, ...basePayload });
      }
      ...
      const sessions = await dbAll(
        `
        SELECT session_id, date, opened_at, closed_at, status
        FROM daily_closings
      ...
      for (const s of sessions || []) {
        const openedAt = String(s.opened_at || '').trim();
        const closedAt = String(s.closed_at || '').trim();
        const isOpen = String(s.status || '').toLowerCase() === 'open';
        const rangeEnd = isOpen ? getLocalDatetimeString() : closedAt || getLocalDatetimeString();
        if (!openedAt) continue;
        const sessionReport = await buildSalesReportPayload(dbGet, dbAll, rangeCfg(openedAt, rangeEnd));
        sessionsOnDay.push({
          session_id: s.session_id,
          business_date: s.date,
          opened_at: openedAt,
          closed_at: isOpen ? null : closedAt || null,
          status: s.status,
          report: sessionReport,
        });
      }

      let calendarDayProvisional = false;
      for (const s of sessions || []) {
        const openDay = String(s.opened_at || '').substring(0, 10);
        const closeDay =
          String(s.status || '').toLowerCase() === 'open'
            ? todayStr
            : String(s.closed_at || '').substring(0, 10) || todayStr;
        if (openDay < D || closeDay > D) {
          calendarDayProvisional = true;
          break;
        }
      }
